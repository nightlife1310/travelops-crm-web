...
const AUTOMATION_RULES = [
  {
    id: "max-new-request",
    title: "Новая заявка из MAX",
    trigger: "Клиент отправил анкету",
    action: "Создать заявку, назначить менеджера, отправить приветствие",
  ...
  },
  {
    ...
    trigger: "Прошло 24 часа после запроса",
    action: "Напомнить клиенту и подсветить заявку менеджеру",
    status: "в CRM",
    tone: "navy",
  },
  {
    id: "payment-reminder",
    title: "Предоплата ожидается",
    trigger: "Статус оплаты не изменился",
    action: "Отправить шаблон оплаты и поставить следующий шаг",
    status: "в CRM",
    tone: "amber",
  },
  {
    ...
    trigger: "В заявке появился рейс или транспорт",
    action: "Создать задачу, предложить свободные автомобили",
    status: "логистика",
    tone: "violet",
  },
  {
    ...
    trigger: "Табло показывает задержку или изменение времени",
  ...
  programs: [
    {
      ...
      price: "от 180 000 ₽",
      manager: "Алина",
      summary: "Классический маршрут для первого знакомства с Сахалином: природа, маяк, море и локальная кухня.",
      description: "Пятидневная программа для туристов, которым нужен спокойный, понятный и красивый маршрут без перегруза. Подходит для пары или небольшой семьи.",
      includes: ["Трансферы по программе", "Экскурсии с гидом", "Подбор проживания", "Рекомендации по ресторанам"],
      itinerary: ["День 1: встреча и заселение", "День 2: обзорная экскурсия", "День 3: природный маршрут", "День 4: море и локальная кухня", "День 5: выезд"],
      photos: ["Фото маяка Анива", "Фото побережья", "Фото отеля"],
      videos: ["Короткое видео маршрута"],
      files: ["PDF-программа для клиента"],
    },
    {
      id: "kurils-sakhalin",
      title: "Курилы + Сахалин",
      dates: "3-11 августа",
      duration: "9 дней",
      hotel: "Azimut Владивосток",
      status: "В работе",
      price: "от 620 000 ₽",
      manager: "Михаил",
      summary: "Большой маршрут с перелетами, островами и аккуратной логистикой для семьи или мини-группы.",
      description: "Программа для клиентов, которые хотят увидеть Курилы и Сахалин в одной поездке. Основной акцент - безопасность, понятные переезды и контроль расписания.",
      includes: ["Маршрутный план", "Подбор перелетов", "Гиды на ключевых точках", "Трансферы аэропорт-отель"],
      itinerary: ["Дни 1-2: Владивосток и перелет", "Дни 3-6: Курильские острова", "Дни 7-8: Сахалин", "День 9: обратный вылет"],
      photos: ["Фото островов", "Фото маршрута", "Фото размещения"],
      videos: ["Видео с маршрута"],
      files: ["PDF-маршрут", "Памятка туриста"],
    },
    {
      id: "busse-yuzhno",
      ...
      price: "от 75 000 ₽",
      manager: "Екатерина",
      summary: "Короткая программа для гостей мероприятия: Южно-Сахалинск, Буссе и вечерняя экскурсия.",
      description: "Компактная программа для туристов, которые приезжают на несколько дней и хотят быстро выбрать дополнительные активности.",
      includes: ["Обзорная экскурсия", "Подбор активностей", "Трансфер по запросу", "Сопровождение менеджера"],
      itinerary: ["День 1: заселение и город", "День 2: лагуна Буссе", "День 3: свободное время и выезд"],
      photos: ["Фото лагуны Буссе", "Фото города"],
    ...
    },
    {
      ...
      price: "от 350 000 ₽",
      manager: "Алина",
      summary: "Индивидуальные выходные с водителем, ресторанами и гибким графиком без группы.",
      ...
      photos: ["Фото автомобиля", "Фото ресторана", "Фото маршрута"],
    ...
    },
    {
      ...
      price: "от 1 200 000 ₽",
      manager: "Михаил",
      summary: "Корпоративная программа с участниками, трансферами, списками и отчетными документами.",
      ...
      itinerary: ["День 1: прибытие группы", "День 2: деловая программа", "День 3: экскурсия и вылет"],
      photos: ["Фото площадки", "Фото транспорта"],
  ...
  templates: [
    {
      ...
      text: "Здравствуйте! Спасибо за заявку, менеджер уже изучает детали и скоро вернется с вариантами.",
    },
    {
      id: "program",
      title: "Выбор программы",
      text: "Подготовили варианты программы. Пожалуйста, отметьте интересующие активности по ссылке.",
    },
    {
      id: "payment",
      title: "Напоминание о предоплате",
      text: "Напоминаем, что для фиксации брони нужно внести предоплату до конца дня.",
    },
  ],
};
...
let airportBoard = {
  ...
  flights: getDemoAirportFlights("departures"),
  loading: false,
  error: "",
  upstreamUrl: "",
  proxyUrl: "",
...
document.addEventListener("click", (event) => {
  ...
  if (action === "assignTransferCar") assignTransferCar(actionEl.dataset.taskId, actionEl.dataset.carId);
  ...
  if (action === "submitClientDemo") createClientDemoRequest();
...
document.addEventListener("drop", (event) => {
  ...
  assignLogisticsItemToCar(logisticsId, zone.dataset.dropCarId);
});
...
document.addEventListener("change", (event) => {
  ...
  if (systemField) updateSystemSetting(systemField.dataset.systemField, systemField.value);
});
...
function ensureStateShape() {
  ...
  state.programs = state.programs.map((program, index) => normalizeProgram(program, index));
  ...
  state.requests.forEach((request, index) => {
    ...
    if (!request.costItems) request.costItems = getDefaultCostItems(request);
...
function applyStartupParams() {
  ...
  const allowedViews = ["dashboard", "requests", "clients", "program", "finance", "arrivals", "departures", "transport", "cars", "messages", "automation", "settings", "journal"];
  if (view === "airport") {
...
function renderDashboard() {
  ...
  const attentionRequests = state.requests.filter((item) => item.status === "new" || item.priority === "Высокий").slice(0, 4);
    ...
    <div class="dashboard-showcase">
      <section class="panel dashboard-automation-panel">
        <div class="panel-header">
          <div>
            <h3>Путь клиента превращается в задачи</h3>
            <p>Клиент заполняет MAX-анкету, а CRM сама раскладывает работу по менеджеру, документам, рейсу и трансферу.</p>
          </div>
          <button class="button primary" type="button" data-action="submitClientDemo">${iconSvg("automation", "button-icon")}<span>Показать сценарий</span></button>
        </div>
        <div class="dashboard-flow">
          <button type="button" data-view="requests"><span>1</span><strong>Заявка</strong><small>клиент и программа</small></button>
          <button type="button" data-view="requests"><span>2</span><strong>Документы</strong><small>запрос и контроль</small></button>
          <button type="button" data-view="arrivals"><span>3</span><strong>Рейс</strong><small>табло и задержки</small></button>
          <button type="button" data-view="cars"><span>4</span><strong>Трансфер</strong><small>задача без машины</small></button>
          <button type="button" data-view="journal"><span>5</span><strong>Журнал</strong><small>всё записано</small></button>
        </div>
      </section>
    </div>
    ...
    <div class="work-grid">
      <section class="panel">
        ...
        <div class="simple-list">
          ...
          `).join("") || ""}
          ${!attentionRequests.length && !tasksWithoutCar.length && !incompleteCars.length ? `<div class="empty-state compact-empty">Критичных задач нет</div>` : ""}
        </div>
      </section>
  ...
  return `
    <section class="phone-preview" aria-label="Пример формы в MAX">
      ...
      <div class="phone-screen">
        <h4>Анкета путешественника</h4>
        <p>Клиент заполняет только то, что запускает работу CRM.</p>
        ...
        <button class="button primary" type="button" data-action="submitClientDemo">Отправить заявку</button>
      </div>
    </section>
  `;
}
  ...
  return `
    <div class="section-heading">
      <div>
        ...
        <p>В MVP клиент создается автоматически из анкеты или ручной заявки.</p>
      </div>
    </div>
    <div class="simple-list">
      ${requests.map((item) => `
  ...
  return `
    ...
    <div class="program-grid">
      ${programs.map((program) => `
        <article class="program-row program-card">
          ...
          <div class="program-actions">
            <button class="chip-button" type="button" data-action="openProgramPreview" data-program-id="${escapeHtml(program.id)}">Отправка</button>
...
function showCarsMetric(metric = "all") {
  ...
  const titleMap = {
    all: ["Автопарк", "Все машины, которые заведены в CRM."],
...
function buildLogisticsItems(flights, isDepartures) {
  const requests = state.requests.length ? state.requests : initialData.requests;
  const sourceFlights = flights.length ? flights : getDemoAirportFlights(ui.airportMode);
  return requests.map((request, index) => {
    const flight = sourceFlights[index % sourceFlights.length];
    const id = `${ui.airportMode}:${ui.airportDate}:${request.id}:${flight.key}`;
    const task = findTaskForLogisticsId(id);
    const car = task?.carId ? getReadyCars().find((item) => item.id === task.carId) : null;
    const noTransfer = !task && index % 5 === 2;
    ...
    return {
      ...
      pax: request.pax,
      city: flight.city,
      hotel: getHotelLabel(request, index),
      ...
      noTransfer,
      direction: isDepartures ? "Выезд в аэропорт" : "Встреча в аэропорту",
    };
  });
}
...
function getMessageInboxItems() {
    ...
    .flatMap((request) => {
      ...
      if (isMax) {
        items.push({
          ...
          requestId: request.id,
          title: request.name,
          text: `MAX-анкета: ${request.route || request.customProgram || "программа уточняется"}${request.flightNo ? ` · рейс ${request.flightNo}` : ""}${request.transferNeeded ? " · нужен трансфер" : ""}`,
  ...
  return requestItems.length ? requestItems : [
    { id: "demo-1", requestId: requests[0]?.id || "", title: "Анна Смирнова", text: "Можно ли добавить маяк Анива?", time: "Сегодня, 11:02", badge: "MAX" },
    { id: "demo-2", requestId: requests[1]?.id || "", title: "Денис Ковалев", text: "Пришлите ссылку на оплату брони.", time: "Вчера, 19:14", badge: "CRM" },
    { id: "demo-3", requestId: requests[2]?.id || "", title: "Марина Волкова", text: "Отметила интересующие мероприятия.", time: "Вчера, 13:22", badge: "MAX" },
  ];
}
    ...
    <div class="work-grid">
      ...
      <section class="panel">
        <div class="panel-header">
          <div>
            <h3>Шаблоны</h3>
            <p>Откройте заявку и отправьте один из шаблонов клиенту.</p>
          </div>
        </div>
        <div class="simple-list">
...
function renderAutomation() {
  const readyCount = AUTOMATION_RULES.filter((rule) => ["в CRM", "логистика"].includes(rule.status)).length;
  ...
  return `
    ...
    <div class="metric-strip automation-strip">
      <button class="metric metric-button" type="button"><span>Сценарии</span><strong>${AUTOMATION_RULES.length}</strong><small>заявки, документы, оплата, логистика</small></button>
      <button class="metric metric-button" type="button"><span>В CRM</span><strong>${readyCount}</strong><small>можно включать без бота</small></button>
      <button class="metric metric-button" type="button"><span>MAX / live</span><strong>${botCount}</strong><small>ждут подключение API</small></button>
      <button class="metric metric-button" type="button"><span>Журнал</span><strong>${getActivities().length}</strong><small>срабатывания пишем туда</small></button>
    </div>

    <section class="panel automation-demo-panel">
      <div class="panel-header">
        <div>
          <h3>MAX-анкета → автоматическая работа CRM</h3>
          <p>Показывает бизнесу главный смысл: клиент заполнил форму, а CRM сама создала действия для менеджера и логистики.</p>
        </div>
        <button class="button primary" type="button" data-action="submitClientDemo">${iconSvg("automation", "button-icon")}<span>Запустить сценарий</span></button>
      </div>
      ...
      <div class="automation-grid">
        ${AUTOMATION_RULES.map((rule) => `
          <button class="automation-card" type="button" data-action="openAutomationRule" data-rule-id="${escapeHtml(rule.id)}">
            ...
            <small>${escapeHtml(rule.trigger)}</small>
      ...
      <div class="modal-card metric-modal">
        ...
        <section class="detail-section">
          <div class="detail-grid">
            ...
            <div class="info-tile"><span>Журнал</span><strong>записывать каждое срабатывание</strong></div>
          </div>
        </section>
        <section class="detail-section">
          <strong>Как внедрять</strong>
          <p class="muted">Сначала фиксируем событие в CRM, потом добавляем действие: сообщение клиенту, задачу менеджеру, изменение статуса или логистическую задачу. После подключения MAX этот же сценарий сможет отправлять сообщения клиенту автоматически.</p>
        </section>
      </div>
    </div>
  `;
  ...
  return `
    <section class="panel max-bot-panel">
      <div class="panel-header">
        <div>
          <h3>MAX-бот</h3>
          <p>Анкета клиента и автоматическое создание работы в CRM</p>
        </div>
      ...
      <div class="simple-list compact-list">
        ...
        <div class="template-row">
          <strong>Сценарий анкеты</strong>
          <p>${fields.map((field) => escapeHtml(field)).join(" · ")}</p>
        </div>
        <div class="template-row">
          <strong>Автоматизация после анкеты</strong>
    ...
    <div class="work-grid">
      ...
      <section class="panel">
        ...
        <div class="form-actions">
          <button class="button secondary" type="button" data-action="resetDemo">${iconSvg("refresh", "button-icon")}<span>Сбросить данные</span></button>
        </div>
      </section>
...
function renderJournal() {
  const events = sortJournalEvents(getActivities()).slice(0, 60);
  ...
  return `
    <section class="detail-section document-section">
      <div class="section-title-row">
        ...
        <button class="chip-button" type="button" data-action="requestDocuments" data-id="${escapeHtml(request.id)}">Запросить все</button>
      </div>
      <div class="document-summary-line">
        ...
        <small>${doneCount}/${docs.length} получено или проверено</small>
      </div>
...
function requestDocuments(id) {
  ...
  const message = `Пожалуйста, пришлите документы: ${requested.join(", ") || "паспортные данные и список туристов"}.`;
  ...
  renderDetail();
  render();
  toast("Запрос документов зафиксирован и добавлен в историю сообщений.");
}
  ...
  detailPanel.innerHTML = `
    ...
    <section class="detail-section">
      ...
      <div class="detail-grid">
        ...
        <div class="info-tile"><span>Оплачено</span><strong>${escapeHtml(item.paidAmount || "0 ₽")}</strong></div>
        <div class="info-tile"><span>Расчет тура</span><strong>${finance.hasCosts ? formatMoneyInput(finance.costs) : "Не заполнен"}</strong></div>
        <div class="info-tile"><span>Оплата</span><strong>${escapeHtml(item.paymentStatus)}</strong></div>
        <div class="info-tile"><span>Документы</span><strong>${escapeHtml(item.documentStatus)}</strong></div>
...
function assignTransferCar(taskId, carId) {
  const task = findTransferTask(taskId);
  if (!task) {
    toast("Задача трансфера не найдена.");
    return;
  }
...
function assignLogisticsItemToCar(logisticsId, carId) {
  const item = findLogisticsItem(logisticsId);
  if (!item) {
  ...
  if (request) {
    ...
    request.messages.unshift(buildTransferNotification(request, task, car));
  }
...
function buildTransferNotification(request, task, car) {
  return [
    `${request.name}, ваш трансфер назначен.`,
    `Рейс: ${task.flightNo}, ${task.date} ${task.time}.`,
    `Автомобиль: ${getCarTitle(car)}, номер ${car.plate}.`,
    `Водитель: ${car.driver}, телефон ${car.phone}.`,
  ].join(" ");
}
  ...
  modalRoot.innerHTML = `
    <div class="modal-backdrop" role="dialog" aria-modal="true" aria-labelledby="carEditTitle">
      <div class="modal-card">
        ...
        <form id="carForm" class="modal-form">
          ...
          <div class="form-actions full">
            ...
            <button class="button primary" type="submit">Сохранить автомобиль</button>
          </div>
        </form>
      </div>
    </div>
...
function sendTemplate(id, templateId) {
  const item = state.requests.find((request) => request.id === id);
  ...
  renderDetail();
  render();
  toast("Сообщение добавлено в историю отправки.");
}
...
function createClientDemoRequest() {
  const request = {
    id: nextId(),
    ...
    tags: ["MAX", "Анкета", "Трансфер"],
    notes: [
      "Заявка создана из MAX-анкеты.",
  ...
  addActivity(`${request.name} отправил анкету через MAX`, { targetType: "request", targetId: request.id, targetView: "requests" });
  ...
  addActivity(`${request.name}: запрошены документы после анкеты`, { targetType: "request", targetId: request.id, targetView: "requests" });
  persist();
  ui.view = "requests";
  syncNav();
  render();
...
function buildRequestFromMaxInbox(item = {}) {
  ...
  const flightNo = cleanMaxValue(item.flightNo).toUpperCase();
  ...
  const request = {
    ...
    comment: cleanMaxValue(item.comment || item.rawText) || "Клиент заполнил анкету в MAX.",
    tags: ["MAX", "Анкета", item.priceReference ? "Цена" : "", flightNo ? "Рейс" : "", transferNeeded ? "Трансфер" : ""].filter(Boolean),
    ...
    messages: [
      ...
      documentStatus === "Запрошены" ? "Пожалуйста, пришлите паспортные данные туристов для оформления." : "Документы отмечены как полученные, менеджер проверит их в CRM.",
    ],
  };
  request.documents = normalizeDocumentChecklist(request);
  request.documentStatus = getOverallDocumentStatus(request.documents);
...
function getMaxAutomationNotes(item = {}, flags = {}) {
  const notes = ["Заявка создана из MAX-анкеты."];
  ...
  if (flags.documentStatus === "Запрошены") notes.push("CRM автоматически поставила задачу запросить документы.");
    ...
    <div class="modal-backdrop" role="dialog" aria-modal="true" aria-labelledby="programFormTitle">
      <div class="modal-card program-modal">
        ...
        <form id="programForm" class="modal-form">
          ...
          <label class="full"><span>Что входит</span><textarea name="includes" placeholder="Каждый пункт с новой строки">${escapeHtml(joinLines(program.includes))}</textarea></label>
          <label class="full"><span>Прайс: входит в стоимость</span><textarea name="priceIncludes" placeholder="Проживание, экскурсии, гид, трансферы...">${escapeHtml(joinLines(program.priceIncludes))}</textarea></label>
          <label class="full"><span>Прайс: оплачивается отдельно</span><textarea name="priceExcludes" placeholder="Авиабилеты, питание вне программы, личные расходы...">${escapeHtml(joinLines(program.priceExcludes))}</textarea></label>
...
function buildProgramFromForm(formData, existing = {}, uploads = []) {
  ...
  return normalizeProgram({
    ...
    priceIncludes: parseLines(formData.get("priceIncludes")),
...
function openProgramPreviewModal(programId) {
  ...
  if (!program) return;
  const message = buildProgramMessage(program);
...
function buildProgramMessage(program) {
  const includes = normalizeLines(program.includes, []).map((item) => `• ${item}`).join("\n");
...
function markProgramSent(programId) {
  const program = getPrograms().find((item) => item.id === programId);
  if (!program) return;
  ...
  if (request) {
    request.messages.unshift(buildProgramMessage(program));
  }
...
function getDefaultCostItems(request) {
  const demo = {
    "TR-1042": { accommodation: "45 000 ₽", transport: "22 000 ₽", guides: "24 000 ₽", tickets: "12 000 ₽", reserve: "5 000 ₽" },
    "TR-1041": { accommodation: "160 000 ₽", transport: "95 000 ₽", guides: "70 000 ₽", tickets: "45 000 ₽", reserve: "20 000 ₽" },
    "TR-1040": { accommodation: "17 000 ₽", transport: "10 000 ₽", guides: "8 000 ₽", tickets: "5 000 ₽", reserve: "2 000 ₽" },
    "TR-1039": { accommodation: "80 000 ₽", transport: "60 000 ₽", guides: "45 000 ₽", tickets: "20 000 ₽", reserve: "10 000 ₽" },
    "TR-1038": { accommodation: "315 000 ₽", transport: "190 000 ₽", guides: "130 000 ₽", tickets: "80 000 ₽", reserve: "45 000 ₽" },
  };
...
function renderRequestFinanceSummary(request) {
  ...
  return `
    <section class="detail-section finance-detail">
      <div class="section-title-row">
        <strong>Финансы и расчет тура</strong>
        <button class="chip-button" type="button" data-action="openRequestEdit" data-id="${escapeHtml(request.id)}">Редактировать расчет</button>
      </div>
...
function renderTourOpsChecklist(request) {
  ...
  return `
    <section class="detail-section">
      <strong>Операционная готовность</strong>
...
function syncNav() {
  document.querySelectorAll(".nav-item, .mobile-tabbar button").forEach((item) => {
    ...
    const isLogistics = targetView === "transport" && ["transport", "arrivals", "departures", "cars"].includes(ui.view);
...
async function loadAirportBoard(options = {}) {
  ...
  try {
    ...
    airportBoard = {
      source: snapshot ? "cached-live" : "demo",
      updatedAt: snapshot ? snapshot.updatedAt : "Live-данные недоступны",
      flights: snapshot ? snapshot.flights : getDemoAirportFlights(ui.airportMode),
      loading: false,
      error: snapshot
...
function getDemoAirportFlights(mode) {
  const departures = [
    {
      no: "SU 1701",
      codeshare: "FV 6201",
...
function isRoleViewEnabled(role, view) {
  const access = getAccessSettings();
  return Boolean(access[role]?.views?.includes(view));
}
